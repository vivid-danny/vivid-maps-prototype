 ...
 */
export function buildConnectorFeatures(
  listings: Listing[],
  coordsBySeatId: Map<string, [number, number]>,
...
/**
 * Populates the seat-connectors dynamic GeoJSON source with LineStrings
 * connecting consecutive seats within each listing.
 *
 * When `showConnectors` is false the source is set to an empty collection, so
 * the connector layers render nothing and expose no hover/click targets.
 */
export function useListingConnectors({
  ...
  useEffect(() => {
    ...
    const features = showConnectors ? buildConnectorFeatures(listings, coordsBySeatId) : [];

    const source = map.getSource(SOURCE_SEAT_CONNECTORS) as GeoJSONSource | undefined;
    if (source) {
      source.setData({ type: 'FeatureCollection', features });
    }
  }, [ready, listings, coordsBySeatId, showConnectors]); // eslint-disable-line react-hooks/exhaustive-deps
}
